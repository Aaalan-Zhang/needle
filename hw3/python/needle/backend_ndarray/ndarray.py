...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # Check that the array is compact first
        if not self.is_compact():
            # If not compact, make it compact first
            compact_array = self.compact()
            return compact_array.reshape(new_shape)
        
        # Check that the total number of elements is the same
        if prod(self.shape) != prod(new_shape):
            raise ValueError("Cannot reshape array - product of dimensions must be the same")
        
        # Calculate new strides for the new shape
        new_strides = self.compact_strides(new_shape)
        
        # Return a new NDArray with the same memory but new shape and strides
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        # Check that new_axes is a valid permutation
        assert len(new_axes) == self.ndim, "new_axes must have same length as number of dimensions"
        assert set(new_axes) == set(range(self.ndim)), "new_axes must be a permutation of dimension indices"
        
        # Create new shape and strides by reordering according to new_axes
        new_shape = tuple(self.shape[i] for i in new_axes)
        new_strides = tuple(self.strides[i] for i in new_axes)
        
        # Return new NDArray with permuted shape and strides
        return NDArray.make(new_shape, strides=new_strides, device=self.device, handle=self._handle, offset=self._offset)
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        # Pad the current shape with 1s on the left to match new_shape length
        old_shape = self.shape
        old_strides = self.strides
        
        # If new_shape has more dimensions, pad with 1s on the left
        if len(new_shape) > len(old_shape):
            # Pad old_shape and old_strides with 1s and 0s respectively on the left
            padded_shape = (1,) * (len(new_shape) - len(old_shape)) + old_shape
            padded_strides = (0,) * (len(new_shape) - len(old_shape)) + old_strides
        else:
            padded_shape = old_shape
            padded_strides = old_strides
        
        # Check broadcasting rules and compute new strides
        new_strides = []
        for i in range(len(new_shape)):
            if padded_shape[i] == new_shape[i]:
                ...
                # Broadcasting from size 1, set stride to 0
                new_strides.append(0)
            else:
                # Invalid broadcast
                raise AssertionError(f"Cannot broadcast dimension {i} from {padded_shape[i]} to {new_shape[i]}")
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        # Compute new shape, strides, and offset based on slices
        new_shape = []
        new_strides = []
        ...
        for i, sl in enumerate(idxs):
            ...
            dim_size = (stop - start + step - 1) // step  # This is ceil((stop - start) / step)
            new_shape.append(dim_size)
            
            # Calculate the new stride for this dimension
            new_strides.append(self.strides[i] * step)
            
            # Add to the offset based on the start index
