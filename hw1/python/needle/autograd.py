...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    ### BEGIN YOUR SOLUTION
    # Process nodes in reverse topological order for backpropagation
    for node in reverse_topo_order:
        # Skip if no gradients have been accumulated for this node
        ...
        # Store gradient in the node's grad field if it requires gradients
        if node.requires_grad:
            node.grad = node_grad
        
        # If this is a leaf node (no operation), skip gradient computation
        if node.op is None:
            continue
            
        # Compute gradients w.r.t. inputs using the operation's gradient method
